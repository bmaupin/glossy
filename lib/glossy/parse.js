...
GlossyParser.prototype.parse = function(rawMessage, callback) {
    ...
    var timeStamp, appSegment;
    // Added month detection to help distinguish RFC 5424 vs RFC 3164. Could be improved further.
    if (/^(<\d+>[0-1])$/.test(segments[0]) && !(/^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)$/.test(segments[1]))) {
        ...
        if(segments[0] !== '-') {
            if (/^\[./.test(segments[0])) { // only proceed to try to process if it is considered a valid form of structured-data
                var spliceMarker = 0;
    ...
    } else if (/^(<\d+>\d+:)$/.test(segments[0])) {
        parsedMessage.type    = 'RFC3164';
        timeStamp             = segments.splice(0,1).join(' ').replace(/^(<\d+>?)/,'');
        parsedMessage.time    = this.parseBsdTime(timeStamp);
        parsedMessage.message = segments.join(' ');

    } else if(/^(<\d+>\w+)/.test(segments[0])) {
        parsedMessage.type    = 'RFC3164';
        if (/^(<\d+>0)$/.test(segments[0])) { // has a version portion which needs to be stripped
            segments[0] = segments[0].replace(/0$/, '') + segments.splice(1,1); // strip the version and concat to the next segment
        }
        if (/^<\d+>\d+-\d+-\d+T\S+/.test(segments[0])) { // parse high precision date (ISO8601 format)
            parsedMessage.time = this.parseTimeStamp(segments.shift().replace(/^(<\d+>(?:0 )?)/,''));
        } else {
            if (segments[1] === '') { segments.splice(1,1); }
            timeStamp             = segments.splice(0,3).join(' ').replace(/^(<\d+>(?:0 )?)/,'');
            parsedMessage.time    = this.parseBsdTime(timeStamp);
        }
        parsedMessage.host    = segments.shift();
        appSegment            = segments.shift();
        if (appSegment && appSegment.substr(-1) === ':') {
            appSegment = appSegment.substring(0, appSegment.length-1).split('[');
            if (appSegment.length === 2) {
                parsedMessage.appName = appSegment[0];
                parsedMessage.pid = appSegment[1].replace(/[\[\]]/, '');
            } else if (appSegment.length === 1) {
                parsedMessage.appName = appSegment[0];
            }
        } else {
            segments.unshift(appSegment);
        }
        parsedMessage.message = segments.join(' ');
    }

    if (callback) {
        // Strip leading space due to bad rsyslog template (see sp-if-no-1st-sp)
        if (parsedMessage.message && parsedMessage.message.indexOf(' ') === 0) {
            parsedMessage.message = parsedMessage.message.substring(1);
        }
    ...
    if(typeof Buffer == 'function') {
        for(var hash in validatedBlock.hashBlock) {
            validatedBlock.hashBlock[hash] = new Buffer(
                validatedBlock.hashBlock[hash], 'base64');
        }
        validatedBlock.thisSignature = new Buffer(
            signedBlock.SIGN, 'base64');
    } else {
